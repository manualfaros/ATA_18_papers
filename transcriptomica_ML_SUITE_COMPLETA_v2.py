...
with tabs[0]:
    ...
    Cada punto es un gen. Se comparan **dos valores por gen**:

    • eje X: **log2 Fold Change (logFC)** → cuánto cambia el gen  
    • eje Y: **−log10(p-valor ajustado)** → cuán improbable es ese cambio por azar

    No se comparan genes entre sí: se evalúa cada gen contra la hipótesis de **“no cambio”**.
    """)
...
with tabs[1]:
    ...
    Un gen se representa como un **vector** con sus valores de logFC a través de todos los contrastes.
    Esta pestaña solo muestra ese vector (tabla + barras): no hay clustering ni modelos.
    """)
...
with tabs[2]:
    ...
    with st.expander("📐 ¿Qué hace este heatmap matemáticamente?", expanded=False):
        st.markdown("""
        Cada gen es un **vector numérico** (logFC por contraste).

        Para comparar genes, calculamos una **distancia** entre sus vectores:
        - genes cercanos → perfiles similares
        - genes lejanos → perfiles distintos

        Si eliges *clustering*, el orden de las filas refleja esa similitud.
        """)
...
with tabs[3]:
    ...
    with st.expander("📐 ¿En qué se diferencia este heatmap?", expanded=False):
        st.markdown("""
        Es la misma idea que el heatmap global, pero restringida a genes **SaSP**.

        Matemáticamente: mismos vectores y mismas distancias, pero en un subconjunto.
        Sirve para ver si los SaSP forman **módulos** o subgrupos coherentes.
        """)
...
with tabs[5]:
    ...
    st.info("""
    🤖 **¿Qué se compara aquí matemáticamente?**

    En ML, cada gen sigue siendo un **vector** de logFC, pero ahora se usa para **predecir**
    una etiqueta (función).

    - *Random Forest*: aprende reglas usando los valores del vector para clasificar.
    - *K-means*: agrupa genes por cercanía a **centroides** (patrones promedio) y luego
      evalúa enriquecimiento funcional por cluster.
    """)
...
with tabs[4]:
    ...
    Se comparan genes **de dos en dos**. Para cada par (SaSP, SAOUHSC) se calcula la
    **correlación de Pearson** entre sus perfiles de logFC.

    • r ≈ +1 → suben/bajan juntos (coexpresión)  
    • r ≈ −1 → comportamiento opuesto (anticorrelación)
    """)
